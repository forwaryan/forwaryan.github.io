...
## 从 Lab4 到 Lab5：解决什么新问题？

Lab4 的 KV 虽然有了容错能力（Raft 复制），但所有数据挤在一个 Raft Group 里。加副本只能提高容错度，不能提高吞吐量和容量。这就像一个公司所有工作都压在一个团队上——人再多也是一个 Leader 拍板。Lab5 把工作拆成多份（分片），交给多个独立团队（Group）并行处理：

- 写入瓶颈：所有写都要经过唯一的 Leader → 拆成多个 Group 各自有 Leader
- 存储瓶颈：所有数据存在每台机器上 → 每个 Group 只存一部分
- 扩展方式：只能加副本（提高容错），不能加容量 → 加 Group 就能水平扩容
...
- **Shard**：key 通过 `key2shard(key)` 映射到 Shard 号（取 `key[0] % NShards`，即首字节取模）
