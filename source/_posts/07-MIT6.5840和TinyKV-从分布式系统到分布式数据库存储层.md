...
flowchart TB
    ...
    ROOT --> MIT0["MIT 6.5840<br/>分布式系统正确性"]
    MIT0 --> MR["MapReduce<br/>任务分发/失败重试"]
    MR --> KVS["KVSrv<br/>RPC 去重/幂等"]
    KVS --> RAFT["Raft<br/>选举/日志/Snapshot"]
    RAFT --> KVRAFT["KVRaft<br/>复制状态机"]
    KVRAFT --> SHARD["ShardKV<br/>配置变更/分片迁移"]

    ROOT --> TINY0["TinyKV<br/>分布式数据库存储层"]
    TINY0 --> RAW["StandaloneKV<br/>Badger/CF/Raw API"]
    RAW --> RK["RaftKV<br/>Ready/raftstore/apply"]
    RK --> MRK["Multi-RaftKV<br/>Region/Peer/Scheduler"]
    MRK --> TXN["Transactions<br/>MVCC/Percolator/2PC"]
...
flowchart TB
    ...
    SC --> ROUTE["根据 key 找 shard<br/>再定位 replica group"]
    ROUTE --> G1["Replica Group 1<br/>Raft 复制 shard 0/1/2"]
    ROUTE --> G2["Replica Group 2<br/>Raft 复制 shard 3/4"]
