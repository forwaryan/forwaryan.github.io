...
{% mermaid %}
flowchart LR
    subgraph MIT["MIT 6.5840：分布式系统正确性"]
        MR["MapReduce\n任务分发/失败重试"] --> KVS["KVSrv\nRPC/去重/幂等"]
        KVS --> RAFT["Raft\n选举/日志/Snapshot"]
        RAFT --> KVRAFT["KVRaft\n复制状态机"]
        KVRAFT --> SHARD["ShardKV\n配置变更/分片迁移"]
    end

    subgraph TINY["TinyKV：分布式数据库存储层"]
        RAW["StandaloneKV\nBadger/CF/Raw API"] --> RK["RaftKV\nReady/raftstore/apply"]
        RK --> MRK["Multi-RaftKV\nRegion/Peer/Scheduler"]
        MRK --> TXN["Transactions\nMVCC/Percolator/2PC"]
    end
...
KVRaft 到 ShardKV 的核心变化，可以用这张 Mermaid 图记：

{% mermaid %}
flowchart TB
    C["Client"] --> SC["ShardCtrler\nQuery 最新配置"]
    SC --> ROUTE["根据 key 找 shard\n再定位 replica group"]
    ROUTE --> G1["Replica Group 1\nRaft 复制 shard 0/1/2"]
    ROUTE --> G2["Replica Group 2\nRaft 复制 shard 3/4"]

    subgraph MIG["配置变更时"]
        CFG["新配置进入各组 Raft log"] --> PULL["新 owner 拉取 shard 数据"]
        PULL --> DEDUP["同步 client 去重表"]
        DEDUP --> GC["旧 owner GC 旧 shard"]
    end

    G1 -. "shard migration" .-> G2
{% endmermaid %}
...
flowchart TB
    ...
    Scheduler["Scheduler/PD 思想\n收集 heartbeat\n生成 balance operator"] -.-> S1
    Scheduler -.-> S2
    Scheduler -.-> S3
{% endmermaid %}
...
{% mermaid %}
flowchart LR
    BEGIN["事务开始\n拿到 start_ts"] --> READ["KvGet/KvScan\n按 start_ts 读快照"]
    READ --> PRE["KvPrewrite\n检查 write conflict 和 lock conflict"]
    PRE --> LOCK["lock CF\n写事务锁"]
    PRE --> DEFAULT["default CF\n写临时 value"]
    LOCK --> COMMIT["KvCommit\n拿到 commit_ts"]
    DEFAULT --> COMMIT
    COMMIT --> WRITE["write CF\n写提交记录"]
...
{% mermaid %}
flowchart TB
    CONFLICT["遇到别的事务遗留 lock"] --> CHECK["KvCheckTxnStatus\n检查 primary lock"]
    CHECK -->|primary 已提交| RESOLVE_COMMIT["KvResolveLock\n提交 secondary locks"]
    CHECK -->|primary 超时/不存在| ROLLBACK["KvBatchRollback\n删除临时 value\n写 rollback 记录"]
    RESOLVE_COMMIT --> CLEAN["状态收敛\n清理锁"]
    ROLLBACK --> CLEAN
{% endmermaid %}
